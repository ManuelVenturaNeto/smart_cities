...
class HeatmapService:
    ...
    def __init__(self):
        ...
        self.transformer = Transformer.from_crs("EPSG:32723", "EPSG:4326")  # UTM 23S to WGS84
        ...
        try:
            ...
            for file_path in file_paths:
                try:
                    ...
                    self.log.error(f"Error loading {file_path.name}: {str(e)}", exc_info=True)
                    continue
            ...
            self.log.info(f"Processed {len(points)} heatmap points from {len(file_paths)} files")
